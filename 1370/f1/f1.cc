...
void QUERY(int l, int r, int &node, int &dist) {
  if (memo.count({l, r})) {
    node = memo[{l, r}].first;
    dist = memo[{l, r}].second;
    return;
  }
  vector<int> nodes;
  for (int i = 0; i < n; ++i)
    if (depth[i] >= l && depth[i] <= r)
...
int main() {
  ...
  while (t--) {
    ...
    int lo = (min_dist + 1) / 2, hi = max_depth;
    while (lo <= hi) {
      int mid = (lo + hi) >> 1;
      int _node, _dist;
      QUERY(mid, mid, _node, _dist);
      if (_dist > min_dist)
        hi = mid - 1;
      else
        lo = mid + 1;
